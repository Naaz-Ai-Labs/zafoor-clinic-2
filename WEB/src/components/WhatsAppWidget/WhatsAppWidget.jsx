    ...
    <>
      <div className={`wa-chat${open ? " open" : ""}`}>
        <div className="wa-chat-head">
          <span>{whatsapp.headerTitle}</span>
          <span className="close" onClick={() => setOpen(false)}>
            ✕
          </span>
        </div>
        <div className="wa-chat-body">{whatsapp.greeting}</div>
        <div className="wa-options">
          {whatsapp.options.map((opt) => (
            <button key={opt.label} className="wa-opt" onClick={() => openOption(opt.message)}>
              {opt.label}
            </button>
          ))}
        </div>
      ...
      <div className="social-fabs">
        ...
        <div className="fab fab-whatsapp" aria-label="Chat on WhatsApp" onClick={() => setOpen((v) => !v)}>
